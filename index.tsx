...
const GuidedLearningIcon = () => <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 0 24 24" width="24px" fill="currentColor"><path d="M0 0h24v24H0z" fill="none"/><path d="M18 2H6c-1.1 0-2 .9-2 2v16c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zM6 4h5v8l-2.5-1.5L6 12V4z"/></svg>;
            ...
            parts: [{ text: `
                <div class="book-writer-intro-content">
                    <div class="book-writer-icon-wrapper">
                        <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 0 24 24" width="24px" fill="currentColor"><path d="M0 0h24v24H0z" fill="none"/><path d="M18 2H6c-1.1 0-2 .9-2 2v16c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zM6 4h5v8l-2.5-1.5L6 12V4z"/></svg>
                    </div>
...
Please click the "Select Project API Key" button below to choose a valid project.
  ...
  const handleSendMessage = async (currentPrompt: string, optionalChatId?: string, optionalChatSession?: ChatSession) => {
    ...
    try {
      ...
      if (isImageGenMode) {
        ...
        modelResponseSources = response.candidates?.[0]?.groundingMetadata?.groundingChunks;
      } else {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      const response = await newChatInstance.sendMessage({ message: updatedUserMessage.parts });
                ...
                <div className="input-form-top">
                    ...
                    {isVideoGenMode && (
                         <div className="video-chip-container">
                            ...
                            <div className="aspect-ratio-toggle">
                                <button type="button" className={videoAspectRatio === '16:9' ? 'active' : ''} onClick={() => setVideoAspectRatio('16:9')}>16:9</button>
                                <button type="button" className={videoAspectRatio === '9:16' ? 'active' : ''} onClick={() => setVideoAspectRatio('9:16')}>9:16</button>
                            </div>
                        </div>
                    )}
                  <textarea
                  ...
                  <div className="bottom-left">
                    <div className="tools-menu-container">
                        <button 
                            ref={toolsBtnRef}
                            type="button" 
                            className={`tools-btn ${(isDeepResearchMode || isImageGenMode || isVideoGenMode) ? 'active' : ''}`}
                        ...
                        {isToolsMenuOpen && (
                            <div ref={toolsMenuRef} className="dropdown-menu tools-menu">
                                <button type="button" className="menu-item" onClick={handleToggleDeepResearchMode}>
                                    <SearchIcon />
                                    <span>{isDeepResearchMode ? 'Exit' : ''} Deep Research</span>
                                </button>
                                <button type="button" className="menu-item" onClick={handleToggleImageGenMode}>
                                    <ImageIcon />
                                    <span>{isImageGenMode ? 'Exit' : ''} Image Generation</span>
                                </button>
                                <button type="button" className="menu-item" onClick={handleToggleVideoGenMode}>
                                    <VideoIcon />
                                    <span>{isVideoGenMode ? 'Exit' : ''} Video Generation</span>
                                </button>
                            </div>
                        )}
                    </div>

                    <button type="button" className="icon-btn" onClick={handleUploadClick}>
                      <ImageIcon />
                    </button>
                  </div>
                  <div className="bottom-right">
                    <div ref={modelBtnRef} className="model-selector-container">
                        ...
                        <div ref={modelMenuRef} className="dropdown-menu model-menu">
                            <button type="button" className="menu-item" onClick={() => handleModelChange('gemini-2.5-flash')}>
                            <span>2.5 Flash</span>
                            </button>
                            <button type="button" className="menu-item" onClick={() => handleModelChange('gemini-2.5-pro')}>
                            <span>2.5 Pro</span>
                            </button>
                        </div>
                        )}
                    ...
                    <button type="button" className={`icon-btn ${isRecording ? 'recording' : ''}`} onClick={handleMicClick}>
                      <MicIcon />
                    </button>
                    <button type="submit" className="icon-btn" disabled={isLoading || (!prompt.trim() && !uploadedImage)}>
                      <SendIcon />
                    </button>
                  </div>
                </div>
